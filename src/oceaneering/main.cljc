(ns oceaneering.main
  "Kotodama WASM entrypoint for the Oceaneering clean-room actor (L4) — Clojure port.
  ...
  facts (`oceaneering.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "oceaneering")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"oceaneering.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "oceaneering-compat" :tier tier :entities entities} 200])
...
(defn start! [] :oceaneering-compat/ready)
